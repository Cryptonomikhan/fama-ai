#!/usr/bin/env python3
"""
Example of using JSON file-based storage for persistent agent sessions with Agno.

This example demonstrates how to use JSON storage to maintain conversation
history across multiple interactions with an agent.

Run this example with:
   python examples/json_storage_example.py
"""
...
STORAGE_DIR = os.path.expanduser("~/agno_json_storage")
...
def main():
    ...
    try:
        ...
        console.print(f"[bold green]✓[/bold green] JSON storage initialized in: {STORAGE_DIR}")
    except Exception as e:
        console.print(f"[bold red]✗[/bold red] Failed to initialize JSON storage: {str(e)}")
        return
    ...
    session_file = os.path.join(STORAGE_DIR, f"{agent.session_id}.json")
    ...
    # Show what the JSON file contains (if it exists)
    session_file = os.path.join(STORAGE_DIR, f"{agent.session_id}.json")
    if os.path.exists(session_file):
        try:
            ...
            console.print(Panel(
                ...
                title=f"Contents of {session_file}",
